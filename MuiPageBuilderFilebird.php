...
class MuiPageBuilderFilebird
{
  ...
  public function registerRestRoutes()
  {
    ...
    // These endpoints must not be authorized by WordPress cookies alone.
    add_filter('rest_authentication_errors', function ($result) use ($namespace) {
      if (!$this->isMediaLibraryRoute($namespace)) {
        return $result;
      }
      ...
      if (empty($auth_header)) {
        return new WP_Error(
          'rest_forbidden',
          __('Missing Authorization header. Use Application Password or JWT authentication.'),
          ['status' => 401]
        );
      }
      return $result;
    ...
    // Stop WordPress cookie nonce checks from rejecting authorized XHR requests.
    add_filter('rest_authentication_errors', function ($result) use ($namespace) {
      if ($result !== null || !$this->isMediaLibraryRoute($namespace)) {
        return $result;
      }
    ...
    register_rest_route($namespace, 'ls', [
      ...
      'permission_callback' => fn() => $this->canUseMediaLibraryApi()
    ]);
  ...
  private function isMediaLibraryRoute(string $namespace): bool
  {
    $route = $GLOBALS['wp']->query_vars['rest_route'] ?? '';
    if ($route) {
      return str_starts_with($route, '/' . $namespace);
    }

    $uri = $_SERVER['REQUEST_URI'] ?? '';

    return strpos($uri, '/' . $namespace) !== false;
  }
  ...
  private function canUseMediaLibraryApi()
  {
